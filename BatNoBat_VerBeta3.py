...
# Keep OpenCV from spinning extra threads (stability on Win/Linux)
try:
    cv2.setNumThreads(1)
...
def save_file_png_wrapper(filepath: str, verdict: str, out_root: Path, cfg: SpectroCfg,
                          *, base_folder: str, recurse: bool):
    sub = "CleanBat" if verdict == "Clean Bat" else ("NoisyBat" if verdict == "Noisy Bat" else "NoBat")
    # Mirror the input folder structure under the verdict folder
    if recurse:
        try:
            rel_parent = Path(filepath).parent.relative_to(base_folder)
        except Exception:
            rel_parent = Path()
    else:
        rel_parent = Path()
    out_png = out_root / sub / rel_parent / (Path(filepath).stem + ".png")
...
def process_chunk(args):
    ...
    MAX_POINTS_FOR_DBSCAN = int(params.get('max_points_for_dbscan', 300000))
    if npts > MAX_POINTS_FOR_DBSCAN:
        return []  # bail out rather than risk pathological clustering
...
def analyze_single_file(filepath, params):
    ...
    step = int((CHUNK_DURATION_S - OVERLAP_S) * sr)
    if step <= 0:
        step = chunk_len  # safety

    with sf.SoundFile(filepath, 'r') as f:
        for i in range(0, total_frames, step):
            f.seek(i)
            # robust, 1-D mono float32; avoids ragged arrays later
            audio = f.read(chunk_len, dtype='float32', always_2d=True)
            if audio.size == 0:
                break
            audio_mono = audio[:, 0].copy()
            tasks.append((audio_mono, sr, thr, i / sr, task_params))
...
def _run_with_timeout(fn, args=(), kwargs=None, timeout_s=180):
    """Soft watchdog: runs fn(*args, **kwargs) in a thread; raises _Timeout on expiry."""
...
class AnalysisWorker(QObject):
    ...
    status = Signal(str)  # new: emits status messages for the UI label
    ...
    def run(self):
        ...
        def _rel_parent(p: str) -> Path:
            if self.params['recurse']:
                try:
                    return Path(p).parent.relative_to(base_folder)
                except Exception:
                    return Path()
            else:
                return Path()

        # Pre-group WAVs by their relative parent folder
        groups = defaultdict(list)
        for p in self.wav_files:
            groups[_rel_parent(p)].append(p)

        overall_results = []  # keep a complete set for the final roll-up if desired
        overall_to_delete = []

        cfg = SpectroCfg(style="fast")

        file_index = 0
        for rel_parent, file_list in groups.items():
            group_results = []
            group_to_delete = []
            group_start = time.time()
            group_bytes = 0

            # Optional: ensure verdict subfolders exist for this group (WAV copies + PNGs mirror already)
            # We still use a single out_root; paths below include rel_parent where needed.

            for filepath in file_list:
                if not self.is_running:
                    break
                self.progress.emit(file_index, len(self.wav_files), Path(filepath).name)
                file_index += 1

                PER_FILE_TIMEOUT_S = float(self.params.get('per_file_timeout_s', 240.0))
                try:
                    res = _run_with_timeout(analyze_single_file, args=(filepath, self.params),
                                            timeout_s=PER_FILE_TIMEOUT_S)
                except _Timeout:
                    res = {"filepath": filepath, "duration_s": 0.0, "counts": {}, "verdict": "No Bat",
                           "snr_ratio": None, "noise_ref_db": None}
                except Exception as e:
                    print(f"[ERROR] analyze_single_file crashed for {filepath}: {e}")
                    res = {"filepath": filepath, "duration_s": 0.0, "counts": {}, "verdict": "No Bat",
                           "snr_ratio": None, "noise_ref_db": None}
                ...
                try:
                    group_bytes += (Path(filepath).stat().st_size if Path(filepath).exists() else 0)
                except Exception:
                    pass

                # PNG (now mirrors rel dirs)
                if self.params['save_pngs']:
                    try:
                        save_file_png_wrapper(
                            filepath, verdict, self.out_root, SpectroCfg(style="fast"),
                ...
                # Copy sorted WAVs (this already mirrors via rel_parent in your code)
                copied_ok = False
                if self.params['save_wavs'] and verdict in ("Clean Bat", "Noisy Bat"):
                    try:
                        sub = "CleanBat" if verdict == "Clean Bat" else "NoisyBat"
                        relp = (Path(filepath).parent.relative_to(self.params['base_folder'])
                                if self.params["recurse"] else Path())
                        dst = self.out_root / sub / relp / Path(filepath).name
                ...
                # Queue deletion decision for this group
                if self.params['delete_unsorted']:
                    if verdict in ("Clean Bat", "Noisy Bat"):
                        if copied_ok:
                            group_to_delete.append(filepath)
                    else:
                        group_to_delete.append(filepath)

            # --- end for filepath in file_list ---

            # 2) Write a sub-report for this subfolder (into a mirrored sub-output dir)
            group_duration = time.time() - group_start
            sub_out = self.out_root / rel_parent
            sub_out.mkdir(parents=True, exist_ok=True)
            write_report_txt(group_results, sub_out, group_duration, group_bytes)

            # 3) Delete originals for this subfolder (after copies + PNGs are done)
            if self.is_running and self.params['delete_unsorted'] and group_to_delete:
                # Optional: if you still want a confirmation per-subfolder, emit the request here.
                # self.request_confirmation.emit(f"Delete {len(group_to_delete)} original files in '{rel_parent}'?", group_to_delete)
                # If you prefer automatic deletion (no dialog), do it directly:
                deleted, failed = 0, 0
                for f in group_to_delete:
                    try:
                        os.remove(f);
                        deleted += 1
                    except OSError as e:
                ...
                self.status.emit(
                    f"Deleted originals in '{rel_parent}' — Deleted: {deleted}, Failed: {failed}."
                )

            overall_to_delete.extend(group_to_delete)
        # --- end for rel_parent, file_list in groups.items() ---

        # All subfolders handled; emit final finished with overall results
        self.finished.emit(overall_results, time.time() - start_time)
    ...
    def on_analysis_finished(self, results, duration):
        ...
        if not (params and params['delete_unsorted'] and any(res['verdict'] != 'No Bat' for res in results)):
            QMessageBox.information(
                self, "Success",
                "Batch analysis is complete.\n\n"
                "Per-subfolder reports are saved inside the mirrored subdirectories of:\n"
                f"{(Path(self.folder_path_edit.text()) / '_bat_no_bat_outputs_pyside')}\n\n"
                f"An overall roll-up report was saved to:\n{report_path}"
            )
